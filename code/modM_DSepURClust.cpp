...
Type objective_function<Type>::operator() ()
{
  ...
  PARAMETER( log_tauEpsUrb );
  PARAMETER( log_tauEpsRur );
  ...
  Type sigmaEpsUrb = exp(Type(-0.5) * log_tauEpsUrb);
  Type sigmaEpsRur = exp(Type(-0.5) * log_tauEpsRur);
  ...
  for(int i = 0; i< nuggetUrbDHS.size(); i++) {
    nuggetLogLik += dnorm(nuggetUrbDHS(i), Type(0), sigmaEpsUrb, true); // N(mean, sd);
  }
  for(int i = 0; i< nuggetRurDHS.size(); i++) {
    nuggetLogLik += dnorm(nuggetRurDHS(i), Type(0), sigmaEpsRur, true); // N(mean, sd);
  }
  jnll -= nuggetLogLik;
  ...
  Type logPriTauEpsUrb = dPCPriTau(log_tauEpsUrb, lambdaTauEps);
  Type logPriTauEpsRur = dPCPriTau(log_tauEpsRur, lambdaTauEps);
  ...
  // get final log density
  jnll -= logPriTau;
  jnll -= logPriTauEpsUrb;
  jnll -= logPriTauEpsRur;
  jnll -= logPriPhi;
  ...
  for (int obsI = 0; obsI < num_iUrbanDHS; obsI++) {
    ...
    for (int intI = 0; intI < n_integrationPointsUrbanDHS; intI++) {
      ...
      latentFieldUrbDHS(thisIndex) = alpha + fe_iUrbanDHS(thisIndex) + w_bym2(areaidxlocUrban(obsI)) + nuggetUrbDHS(obsI);
  ...
  for (int obsI = 0; obsI < num_iRuralDHS; obsI++) {
    ...
    for (int intI = 0; intI < n_integrationPointsRuralDHS; intI++) {
      ...
      latentFieldRurDHS(thisIndex) = alpha + fe_iRuralDHS(thisIndex) + w_bym2(areaidxlocRural(obsI)) + nuggetRurDHS(obsI);
  ...
  REPORT(secondPt);
  REPORT(thirdPt);
  REPORT(logPriTauEpsUrb);
  REPORT(logPriTauEpsRur);
  REPORT(KLD);
  REPORT(lexpDensity);
  REPORT(ljacobian);
  REPORT(d);
