#!/usr/bin/env Rscript
# Run the combined MICS+DHS FE+nug model on real data
library(TMB)
...
# build inputs (will load saved intPts for canonical KDHS=16/KDHSr=21)
inputsMDM <- makeInputsMDM(datDHS=ed, datMICS=edMICS, KMICS=KMICS, KDHSurb=KDHSu, KDHSrur=KDHSr)
...
# Use canonical saved DHS integration points for covariates (drop intercept)
load('savedOutput/global/intPtsDHS_16_21.RData') # loads intPtsDHS
intD_saved <- intPtsDHS
XUrbD <- as.matrix(intD_saved$covsUrb)[, -1, drop=FALSE]
XRurD <- as.matrix(intD_saved$covsRur)[, -1, drop=FALSE]
colnames(XUrbD) <- covs
colnames(XRurD) <- covs
...
data_comb <- list(
  ...
  X_betaUrbanDHS = XUrbD[, covs],
  X_betaRuralDHS = XRurD[, covs],
  wUrbanDHS = intD_saved$wUrban, wRuralDHS = intD_saved$wRural,
...
cat('Running combined model on real data...\n')
...
cat('\nCombined model results (real data):\n')
