...
Type objective_function<Type>::operator() ()
{
  ...
  DATA_IVECTOR(areaidxlocUrban); // vector of length n_iUrbanDHS of areal indices
  DATA_IVECTOR(areaidxlocRural); // vector of length n_iRuralDHS of areal indices
  DATA_MATRIX( X_betaUrbanDHS );  // nObsUrban x nPar design matrix. Indexed mod numObsUrban
  ...
  vector<Type> wUrbDHS(num_iUrbanDHS);
  vector<Type> wRurDHS(num_iRuralDHS);
  vector<Type> lliksUrbDHS(num_iUrbanDHS);
  vector<Type> lliksRurDHS(num_iRuralDHS);
  // Type EpsInd = 0;
  
  for (int obsI = 0; obsI < num_iUrbanDHS; obsI++) {
    
    // latent field estimate at each obs
    // EpsInd = areaidxlocUrban(obsI);
    wUrbDHS(obsI) = w_bym2(areaidxlocUrban(obsI));
    latentFieldUrbDHS(obsI) = alpha + fe_iUrbanDHS(obsI) + wUrbDHS(obsI) + nuggetUrbDHS(obsI);
    ...
    lliksUrbDHS(obsI) = dbinom_robust( y_iUrbanDHS(obsI), n_iUrbanDHS(obsI), latentFieldUrbDHS(obsI), true);
    jnll -= lliksUrbDHS(obsI);
  ...
  for (int obsI = 0; obsI < num_iRuralDHS; obsI++) {
    ...
    // latent field estimate at each obs
    // EpsInd = areaidxlocRural(obsI);
    wRurDHS(obsI) = w_bym2(areaidxlocRural(obsI));
    latentFieldRurDHS(obsI) = alpha + fe_iRuralDHS(obsI) + wRurDHS(obsI) + nuggetRurDHS(obsI);
    
    // Uses the dbinom_robust function, which takes the logit probability
    // liksRurDHS(obsI) = dbinom_robust( y_iRuralDHS(obsI), n_iRuralDHS(obsI), latentFieldRurDHS(obsI), false);
    // jnll -= log(liksRurDHS(obsI));
    lliksRurDHS(obsI) = dbinom_robust( y_iRuralDHS(obsI), n_iRuralDHS(obsI), latentFieldRurDHS(obsI), true);
    jnll -= lliksRurDHS(obsI);
