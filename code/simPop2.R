...
#' 
#' adm1proj <- sp::spTransform(adm1, CRS(p4s))
#' adm2proj <- sp::spTransform(adm2, CRS(p4s))
#' 
...
simPopCustom = function(logitRiskDraws, sigmaEpsilonDraws, easpa, popMat, targetPopMat, 
  ...
  # get area names, make sure they and subareas are in alphabetical order
  areas = sort(unique(popMat$area))
  if(any(areas != easpa$area))
    stop("area names in easpa do not match popMat or are not in alphabetical order")
  
  # check to make sure subarea names are in alphabetical order and match up
  if(!is.null(poppsub) && subareaLevel) {
    subareas = sort(unique(poppsub$subarea))
    if(any(subareas != poppsub$subarea) || any(subareas != sort(unique(popMat$subarea)))) {
      stop("subarea names in poppsub do not match popMat or are not in alphabetical order")
    }
  }
...
#' @describeIn simPopInternal For recombining separate multinomials over 
...
#' @describeIn simPopInternal For recombining separate multinomials over 
#' multiple areas into the draws over all grid points
getSortIndicesSubarea = function(i, urban=TRUE, popMat, stratifyByUrban=TRUE, validationPixelI=NULL) {
...
rMyMultinomial = function(n, i, stratifyByUrban=TRUE, urban=TRUE, popMat=NULL, easpa=NULL, min1PerSubarea=FALSE, 
  ...
  } else if(sum(includeI) == 0){
    stop(paste0("no valid pixels to put EAs in for area ", as.character(areas[i]), " and urban level ", urban))
  }
  if(sum(includeI) == 0)
    return(matrix(nrow=0, ncol=n))
...
rmultinom1 = function(n=1, size, prob, maxSize=5000*5000, method=c("mult1", "mult", "indepMH"), verbose=FALSE, minSample=100, 
  ...
  if(method == "mult1") {
    ...
    while(any(is.na(samples))) {
      ...
      thisSamples = matrix(1 + stats::rmultinom(thisNumberOfSamples, size-k, prob=prob), nrow=k)
      ...
      thisSamples = matrix(thisSamples[,u<thisProbs], nrow=k)
      
      # remove excess samples if necessary
      totalSamples = ncol(thisSamples) + n - samplesLeft
      if(totalSamples > n) {
        thisSamples = matrix(thisSamples[,1:samplesLeft], nrow=k)
      }
