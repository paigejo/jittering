...
if(FALSE) {
  ...
  dyn.unload( dynlib("code/modM_DMSepVarClustUrb"))
  compile( "code/modM_DMSepVarClustUrb.cpp", 
...
rand_effs <- c('alpha', 'beta', 'diffUrbMICS', 'w_bym2Star', 'u_bym2Star', 
...
doIHME = FALSE
if(doIHME) {
  data_full$wUrbanDHS[,1] = 1
  data_full$wUrbanDHS[,-1] = 0
  randIUrb = apply(data_full$wUrbanMICS, 1, function(x) {sample(1:length(x), size=1, prob=x)})
  randIRur = apply(data_full$wRuralMICS, 1, function(x) {sample(1:length(x), size=1, prob=x)})
  for(i in 1:nrow(data_full$wUrbanMICS)) {
    data_full$wUrbanMICS[i,randIUrb[i]] = 1
    data_full$wUrbanMICS[i,-randIUrb[i]] = 0
  }
  for(i in 1:nrow(data_full$wUrbanMICS)) {
    data_full$wRuralMICS[i,randIRur[i]] = 1
    data_full$wRuralMICS[i,-randIRur[i]] = 0
  }
}
...

{
  ...
  # optimization took 10.9890666666669 minutes (for IHME model=TRUE)
}
...
# IHME model with urb effect and cluster var differences
# \begin{table}[ht]
# \centering
