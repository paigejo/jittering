...
fitMDM_old = function(datDHS=ed, datMICS=edMICS, inputsMDM=NULL, 
  ...
  if(ncol(intPtsMICS$XUrb) != ncol(intPtsDHS$covsUrb)) {
    stop(paste0("MICS and DHS covariate dimensions differ (",
                ncol(intPtsMICS$XUrb), " vs ", ncol(intPtsDHS$covsUrb),
                "). Ensure both use the same covariate specification."))
  }
  ...
  if(!repar) {
    ...
    if(!file.exists("code/modM_DMSep.so")) {
      print("compiling code/modM_DMSep.cpp...")
    ...
    # compile dynlib if need be
    if(!file.exists("code/modM_DMSepRepar.so")) {
      print("compiling code/modM_DMSepRepar.cpp...")
      compile( "code/modM_DMSepRepar.cpp", 
               framework="TMBad", safebounds=FALSE)
    }
...
fitMDM = function(datDHS=ed, datMICS=edMICS, inputsMDM=NULL,
                  intPtsMICS=NULL, intPtsDHS=NULL,
                  KMICS=100,
                  ...
                  Qgh=10, getSDs=TRUE, doMCMC=FALSE, verbose=FALSE, ...) {

  if(doMCMC) stop("doMCMC not implemented for GH fusion fitMDM")
  ...
  fit_fe = fitFEMD(datDHS=datDHS, datMICS=datMICS, inputsMDM=inputsMDM,
                   alpha_pri=alpha_pri, beta_pri=beta_pri,
                   pc.expPrec=pc.expPrec,
  ...
  unloadDynlibs()
  if(!any(file.exists(paste0("code/modMDM_BYM2_GH_v2", c(".o", ".so", ".dll"))))) {
    compile("code/modMDM_BYM2_GH_v2.cpp", framework="TMBad", safebounds=FALSE)
  }
