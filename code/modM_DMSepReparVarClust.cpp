...
Type objective_function<Type>::operator() ()
{
  ...
  PARAMETER( log_tauEpsUMICS );
  PARAMETER( log_tauEpsRMICS );
  PARAMETER( log_tauEpsUDHS );
  PARAMETER( log_tauEpsRDHS );
  ...
  Type sigmaEpsUMICS = exp(Type(-0.5) * log_tauEpsUMICS);
  Type sigmaEpsRMICS = exp(Type(-0.5) * log_tauEpsRMICS);
  Type sigmaEpsUDHS = exp(Type(-0.5) * log_tauEpsUDHS);
  Type sigmaEpsRDHS = exp(Type(-0.5) * log_tauEpsRDHS);
  ...
  for(int i = 0; i< nuggetUrbMICS.size(); i++) {
    nuggetLogLik += dnorm(nuggetUrbMICS(i), Type(0), sigmaEpsUMICS, true); // N(mean, sd);
  }
  for(int i = 0; i< nuggetRurMICS.size(); i++) {
    nuggetLogLik += dnorm(nuggetRurMICS(i), Type(0), sigmaEpsRMICS, true); // N(mean, sd);
  }
  for(int i = 0; i< nuggetUrbDHS.size(); i++) {
    nuggetLogLik += dnorm(nuggetUrbDHS(i), Type(0), sigmaEpsUDHS, true); // N(mean, sd);
  }
  for(int i = 0; i< nuggetRurDHS.size(); i++) {
    nuggetLogLik += dnorm(nuggetRurDHS(i), Type(0), sigmaEpsRDHS, true); // N(mean, sd);
  }
  jnll -= nuggetLogLik;
  ...
  Type logPriTauEpsUMICS = dPCPriTau(log_tauEpsUMICS, lambdaTauEps);
  Type logPriTauEpsRMICS = dPCPriTau(log_tauEpsRMICS, lambdaTauEps);
  Type logPriTauEpsUDHS = dPCPriTau(log_tauEpsUDHS, lambdaTauEps);
  Type logPriTauEpsRDHS = dPCPriTau(log_tauEpsRDHS, lambdaTauEps);
  ...
  // get final log density
  jnll -= logPriTau;
  jnll -= logPriTauEpsUMICS;
  jnll -= logPriTauEpsRMICS;
  jnll -= logPriTauEpsUDHS;
  jnll -= logPriTauEpsRDHS;
  jnll -= logPriPhi;
  ...
  for (int obsI = 0; obsI < num_iUrbanMICS; obsI++) {
    ...
    for (int intI = 0; intI < n_integrationPointsUrbanMICS; intI++) {
      ...
      latentFieldUrbMICS(thisIndex) = fe_iUrbanMICS(thisIndex) + w_bym2Star(areaidxlocUrbanMICS(obsI)) + nuggetUrbMICS(obsI);
  ...
  for (int obsI = 0; obsI < num_iRuralMICS; obsI++) {
    ...
    for (int intI = 0; intI < n_integrationPointsRuralMICS; intI++) {
      ...
      // latent field estimate at each obs
      latentFieldRurMICS(thisIndex) = fe_iRuralMICS(thisIndex) + w_bym2Star(areaidxlocRuralMICS(obsI)) + nuggetRurMICS(obsI);
  ...
  for (int obsI = 0; obsI < num_iUrbanDHS; obsI++) {
    ...
    for (int intI = 0; intI < n_integrationPointsUrbanDHS; intI++) {
      ...
      // latent field estimate at each obs
      latentFieldUrbDHS(thisIndex) = fe_iUrbanDHS(thisIndex) + w_bym2Star(areaidxlocUrbanDHS(obsI)) + nuggetUrbDHS(obsI);
  ...
  for (int obsI = 0; obsI < num_iRuralDHS; obsI++) {
    ...
    for (int intI = 0; intI < n_integrationPointsRuralDHS; intI++) {
      ...
      // latent field estimate at each obs
      latentFieldRurDHS(thisIndex) = fe_iRuralDHS(thisIndex) + w_bym2Star(areaidxlocRuralDHS(obsI)) + nuggetRurDHS(obsI);
  ...
  REPORT(secondPt);
  REPORT(thirdPt);
  REPORT(logPriTauEpsUMICS);
  REPORT(logPriTauEpsRMICS);
  REPORT(logPriTauEpsUDHS);
  REPORT(logPriTauEpsRDHS);
  REPORT(KLD);
  REPORT(lexpDensity);
  REPORT(ljacobian);
  REPORT(d);
