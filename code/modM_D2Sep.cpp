...
Type objective_function<Type>::operator() ()
{
  ...
  for (int obsI = 0; obsI < num_iUrbanDHS; obsI++) {
    ...
    for (int intI = 0; intI < n_integrationPointsUrbanDHS; intI++) {
      ...
      latentFieldUrbDHS(thisIndex) = alpha + fe_iUrbanDHS(thisIndex) + w_bym2(areaidxlocUrban(thisIndex)) + nuggetUrbDHS(obsI);
  ...
  for (int obsI = 0; obsI < num_iRuralDHS; obsI++) {
    ...
    for (int intI = 0; intI < n_integrationPointsRuralDHS; intI++) {
      ...
      latentFieldRurDHS(thisIndex) = alpha + fe_iRuralDHS(thisIndex) + w_bym2(areaidxlocRural(thisIndex)) + nuggetRurDHS(obsI);
