...
if(FALSE) {
  ...
  # put weight only on the simulated locations
  intPtsMICS$wUrban[,-1] = 0
  intPtsMICS$wUrban[,1] = 1
  intPtsMICS$wRural[,-1] = 0
  intPtsMICS$wRural[,1] = 1
  
  # TODO update the covariates to the ones from the simulated locations for the first 
  # column
  intPtsMICS$XUrb[1:sum(edMICS$urban),] = simEdMICS[[1]][simEdMICS[[1]]$urban,][c("urb", "access", "elev", "distRiversLakes", "pop")]
  intPtsMICS$XRur[1:sum(!edMICS$urban),] = simEdMICS[[1]][!simEdMICS[[1]]$urban,][c("urb", "access", "elev", "distRiversLakes", "pop")]
