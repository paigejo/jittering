...
# similar to generateJobList, except more specialized for the validation 
# of this project. Excludes either areal or cluster level validation 
# files depending on the value of the input "areal".
# iNames: vector of names corresponding to i job index from getJobIndices
# jNames: vector of names corresponding to j job index from getJobIndices
# NOTE: iNames and jNames must be names that uniquely characterize files 
#       with the i and j index respectively
generateJobListValidation = function(workDir="savedOutput/validation/folds/", 
  ...
  if(!areal) {
    excludeFiles = c(excludeFiles, 
                     c("d2areal_", "D2areal_", "dm2areal_", "DM2areal_"))
  } else {
    excludeFiles = c(excludeFiles, 
                     c("d2_", "D2_", "dm2_", "DM2_"))
  }
