...
// This resolves the (phi, u) funnel by constraining both w and u.
//
// include libraries
#include <TMB.hpp>
...
Type objective_function<Type>::operator() ()
{
  ...
  // BYM2 effects: n-1 free parameters each, nth derived as -sum(free)
  ...
  int nFree = w_bym2Free.size();    // n-1
  int nAreas = nFree + 1;           // n
  ...
  w_bym2(nFree) = -wSum;  // nth element = -sum of free
  u_bym2(nFree) = -uSum;
  ...
  // Quadratic form: x^T Q_x x using the full n-vectors
  // Same as Sep model: tau/(1-phi) w^T w + u^T Q u + phi/(1-phi) u^T u - 2 sqrt(phi*tau)/(1-phi) u^T w
  ...
  // Log determinant for the constrained precision
  // For the unconstrained 2n model: n*log((1-phi)/tau) (up to constant)
  // For the constrained 2(n-1) model with sum-to-zero on both w and u: (n-1)*log((1-phi)/tau)
  // (The Schur complement reduces the phi-tau dependent part by one degree of freedom per constraint)
  Type logDet = Type(0.0);
  for(int i = 0; i < gammaTildesm1.size(); i++) {
    logDet += log(Type(1.0) + phi*gammaTildesm1(i));
  }
  Type logDetTau = Type(nFree) * log((Type(1.0) - phi)/tau); // (n-1) instead of n
  
  // BYM2 log density (dropping 2*pi and Q-only constants)
  ...
  for (int obsI = 0; obsI < num_iUrbanMICS; obsI++) {
    ...
    for (int intI = 0; intI < n_integrationPointsUrbanMICS; intI++) {
      ...
      // latent field: alpha + X*beta + w_bym2[area] + nugget
