...
Type objective_function<Type>::operator() ()
{
  ...
  vector<Type> fe_iUrbanMICS(num_iUrbanMICS * n_integrationPointsUrbanMICS); // initialize
  vector<Type> fe_iRuralMICS(num_iRuralMICS * n_integrationPointsRuralMICS);
  
  vector<Type> fe_iUrbanDHS(num_iUrbanDHS * n_integrationPointsUrbanDHS);
  vector<Type> fe_iRuralDHS(num_iRuralDHS * n_integrationPointsRuralDHS);
  vector<Type> testUrbanMICS = (X_betaUrbanMICS * beta.matrix()).col(0); // initialize
  vector<Type> testRuralMICS = (X_betaRuralMICS * beta.matrix()).col(0);
  vector<Type> testUrbanDHS = (X_betaUrbanDHS * beta.matrix()).col(0);
  vector<Type> testRuralDHS = (X_betaRuralDHS * beta.matrix()).col(0);
  for(int i = 0; i < fe_iUrbanMICS.size(); i++) {
    // fe_iUrbanMICS(i) = fe_iUrbanMICS(i) + alpha;
    fe_iUrbanMICS(i) = alpha + beta(0);
    testUrbanMICS(i) += alpha;
  }
  for(int i = 0; i < fe_iRuralMICS.size(); i++) {
    fe_iRuralMICS(i) = alpha;
    testRuralMICS(i) += alpha;
  }
  for(int i = 0; i < fe_iUrbanDHS.size(); i++) {
    fe_iUrbanDHS(i) = alpha + beta(0);
    testUrbanDHS(i) += alpha;
  }
  for(int i = 0; i < fe_iRuralDHS.size(); i++) {
    fe_iRuralDHS(i) = alpha;
    testRuralDHS(i) += alpha;
  }
  ...
  vector<Type> obsLiksUrbDHS(num_iUrbanDHS);
  vector<Type> obsLiksRurDHS(num_iRuralDHS);
  vector<Type> obsLiksUrbMICS(num_iUrbanMICS);
  vector<Type> obsLiksRurMICS(num_iRuralMICS);
  for (int obsI = 0; obsI < num_iUrbanMICS; obsI++) {
    ...
    for (int intI = 0; intI < n_integrationPointsUrbanMICS; intI++) {
      ...
      latentFieldUrbMICS(thisIndex) = fe_iUrbanMICS(thisIndex);
      ...
      if(thisWeight > 0) {
        ...
        liksUrbMICS(obsI, intI) = dbinom( y_iUrbanMICS(obsI), n_iUrbanMICS(obsI), 1/(1+exp(-latentFieldUrbMICS(thisIndex))), false);
    ...
    if(thislik > 0) {
      obsLiksUrbMICS(obsI) = thislik;
      jnll -= log(thislik);
    }
    else {
      obsLiksUrbMICS(obsI) = 1;
    }
  } // for( obsI )
  ...
  for (int obsI = 0; obsI < num_iUrbanDHS; obsI++) {
    ...
    for (int intI = 0; intI < n_integrationPointsUrbanDHS; intI++) {
      ...
      latentFieldUrbDHS(thisIndex) = fe_iUrbanDHS(thisIndex);
      ...
      if(thisWeight > 0) {
        ...
        liksUrbDHS(obsI, intI) = dbinom( y_iUrbanDHS(obsI), n_iUrbanDHS(obsI), 1/(1+exp(-latentFieldUrbDHS(thisIndex))), false);
    ...
    if(thislik > 0) {
      obsLiksUrbDHS(obsI) = thislik;
      jnll -= log(thislik);
    }
    else {
      obsLiksUrbDHS(obsI) = 1;
    }
  } // for( obsI )
  ...
  for (int obsI = 0; obsI < num_iRuralMICS; obsI++) {
    ...
    for (int intI = 0; intI < n_integrationPointsRuralMICS; intI++) {
      ...
      latentFieldRurMICS(thisIndex) = fe_iRuralMICS(thisIndex);
      ...
      if(thisWeight > 0) {
        ...
        liksRurMICS(obsI, intI) = dbinom( y_iRuralMICS(obsI), n_iRuralMICS(obsI), 1/(1+exp(-latentFieldRurMICS(thisIndex))), false);
    ...
    if(thislik > 0) {
      obsLiksRurMICS(obsI) = thislik;
      jnll -= log(thislik);
    }
    else {
      obsLiksRurMICS(obsI) = 1;
    }
  } // for( obsI )
  ...
  for (int obsI = 0; obsI < num_iRuralDHS; obsI++) {
    for (int intI = 0; intI < n_integrationPointsRuralDHS; intI++) {
      ...
      latentFieldRurDHS(thisIndex) = fe_iRuralDHS(thisIndex);
      ...
      if(thisWeight > 0) {
        ...
        liksRurDHS(obsI, intI) = dbinom( y_iRuralDHS(obsI), n_iRuralDHS(obsI), 1/(1+exp(-latentFieldRurDHS(thisIndex))), false);
    ...
    if(thislik > 0) {
      obsLiksRurDHS(obsI) = thislik;
      jnll -= log(thislik);
    }
    else {
      obsLiksRurDHS(obsI) = 1;
    }
  } // for( obsI )
  ...
  REPORT(testUrbanMICS);
  REPORT(testRuralMICS);
  REPORT(testUrbanDHS);
  REPORT(testRuralDHS);
  ...
  REPORT(fe_iUrbanDHS);
  REPORT(fe_iRuralDHS);
  REPORT(obsLiksUrbDHS);
  REPORT(obsLiksRurDHS);
  REPORT(obsLiksUrbMICS);
  REPORT(obsLiksRurMICS);
