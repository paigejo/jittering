...
if(FALSE) {
  ...
  save(areaidxlocUrban, areaidxlocRural, intPtsDHS, 
       ...
       file="savedOutput/global/edSimpleSepInputs.RData")
  
  # compile model ----
  # dyn.unload( dynlib("code/modBYM2JitterDHS2sparse"))
  # compile( "code/modBYM2JitterDHS2sparse.cpp", framework="TMBad", safebounds=FALSE)
  
  dyn.unload( dynlib("code/modBYM2SimpleSep"))
  compile("code/modBYM2SimpleSep.cpp", 
...
rand_effs <- c('alpha', 'beta', 'w_bym2Star', 'u_bym2Star', 'nuggetUrbDHS', 'nuggetRurDHS')
...
# obj$par = optPar
{
  ...
  # optimization took 14.3690333333333 minutes (for intern=FALSE)
}
...
save(SD0, obj, totalTime, sdTime, file="savedOutput/ed/fitSimpleSep2.RData")
out = load("savedOutput/ed/fitSimpleSep2.RData")

testRep = obj$report()
plotMapDat(adm2, expit(testRep$u_bym2 + testRep$v_bym2 + obj$env$last.par[1]), varAreas=adm2$NAME_2, 
...
# \hline
# (Int) & -1.77 & -1.95 & -1.89 & -1.64 & -1.58 \\ 
# urb & 0.30 & 0.08 & 0.15 & 0.44 & 0.52 \\ 
# access & -0.19 & -0.29 & -0.25 & -0.12 & -0.07 \\ 
# elev & 0.17 & 0.02 & 0.07 & 0.27 & 0.32 \\ 
# distRiversLakes & 0.04 & -0.12 & -0.06 & 0.14 & 0.20 \\ 
# popValsNorm & 0.82 & 0.62 & 0.68 & 0.95 & 1.03 \\ 
# sigmaSq & 0.99 & 0.73 & 0.81 & 1.18 & 1.29 \\ 
# phi & 0.93 & 0.74 & 0.85 & 0.99 & 0.99 \\ 
# sigmaEpsSq & 0.93 & 0.77 & 0.82 & 1.04 & 1.10 \\ 
# \hline
# \end{tabular}
# \end{table}
save(gridPreds, file="savedOutput/ed/gridPredsSimpleSep2.RData")
out = load("savedOutput/ed/gridPredsSimpleSep2.RData")
...
save(stratPreds, file="savedOutput/ed/stratPredsSimpleSep2.RData")
save(admin1Preds, file="savedOutput/ed/admin1PredsSimpleSep2.RData")
save(admin2Preds, file="savedOutput/ed/admin2PredsSimpleSep2.RData")
out = load("savedOutput/ed/stratPredsSimpleSep2.RData")
out = load("savedOutput/ed/admin1PredsSimpleSep2.RData")
out = load("savedOutput/ed/admin2PredsSimpleSep2.RData")
...
plotPreds(SD0, obj, popMat=popMatNGAThresh, 
          ...
          plotNameRoot="edSimpleSep2")
plotPreds(SD0, obj, popMat=popMatNGAThresh, 
          gridPreds=gridPreds, arealPreds=stratPreds, 
          plotNameRoot="edSimpleSep2", plotNameRootAreal="Strat")
plotPreds(SD0, obj, popMat=popMatNGAThresh, 
          gridPreds=gridPreds, arealPreds=admin1Preds, 
          plotNameRoot="edSimpleSep2", plotNameRootAreal="Admin1")
plotPreds(SD0, obj, popMat=popMatNGAThresh, 
          gridPreds=gridPreds, arealPreds=admin2Preds, 
          plotNameRoot="edSimpleSep2", plotNameRootAreal="Admin2")
