...
Type objective_function<Type>::operator() ()
{
  ...
  DATA_MATRIX( X_betaUrbanMICS );  // (nStrat * nIntegrationPointsUrban) x nPar design matrix. Indexed mod numObsUrban
  DATA_MATRIX( X_betaRuralMICS );  // first nObsRural rows correspond to first int pt
  DATA_ARRAY( wUrbanMICS ); // nStrat x nIntegrationPointsUrban weight matrix
  DATA_ARRAY( wRuralMICS ); // nStrat x nIntegrationPointsRural weight matrix
  ...
  for (int obsI = 0; obsI < num_iUrbanMICS; obsI++) {
    ...
    for (int intI = 0; intI < n_integrationPointsUrbanMICS; intI++) {
      ...
      latentFieldUrbMICS(thisIndex) = alpha + fe_iUrbanMICS(areaidxlocUrbanMICS(obsI)*n_integrationPointsUrbanMICS + intI) + w_bym2(areaidxlocUrbanMICS(obsI)) + nuggetUrbMICS(obsI);
      // latentFieldUrbMICS(thisIndex) = fe_iUrbanMICS(thisIndex) + projepsilon_iUrbanMICS(obsI);
      
      // and add data contribution to jnll
      // get integration weight
      thisWeight = wUrbanMICS(areaidxlocUrbanMICS(obsI),intI);
  ...
  for (int obsI = 0; obsI < num_iRuralMICS; obsI++) {
    ...
    for (int intI = 0; intI < n_integrationPointsRuralMICS; intI++) {
      ...
      latentFieldRurMICS(thisIndex) = alpha + fe_iRuralMICS(areaidxlocRuralMICS(obsI)*n_integrationPointsRuralMICS + intI) + w_bym2(areaidxlocRuralMICS(obsI)) + nuggetRurMICS(obsI);
      // latentFieldRurMICS(thisIndex) = fe_iRuralMICS(thisIndex) + projepsilon_iRuralMICS(obsI);
      
      // and add data contribution to jnll
      // get integration weight
      thisWeight = wRuralMICS(areaidxlocRuralMICS(obsI),intI);
