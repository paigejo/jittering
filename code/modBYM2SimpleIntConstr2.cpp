...
Type objective_function<Type>::operator() ()
{
  ...
  for (int obsI = 0; obsI < num_iUrbanDHS; obsI++) {
    ...
    // latentFieldUrbDHS(obsI) = alpha + projepsilon_iUrbanDHS(obsI) + nuggetUrbDHS(obsI);
    latentFieldUrbDHS(obsI) = alpha + transformedEpsilon(areaidxlocUrban(obsI)) + nuggetUrbDHS(obsI);
  ...
  for (int obsI = 0; obsI < num_iRuralDHS; obsI++) {
    ...
    // latent field estimate at each obs
    // latentFieldRurDHS(obsI) = alpha + projepsilon_iRuralDHS(obsI) + nuggetRurDHS(obsI);
    latentFieldRurDHS(obsI) = alpha + transformedEpsilon(areaidxlocRural(obsI)) + nuggetRurDHS(obsI);
