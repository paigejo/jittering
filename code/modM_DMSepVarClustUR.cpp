...
Type objective_function<Type>::operator() ()
{
  ...
  PARAMETER( diffUrbMICS ); // MICS urban effect is the regular plus diffUrbMICS
  PARAMETER( diffRurMICS ); // MICS rural level is the regular plus diffUrbMICS
  ...
  jnll -= dnorm(diffUrbMICS, beta_pri(0), beta_pri(1), true); // N(mean, sd)
  jnll -= dnorm(diffRurMICS, beta_pri(0), beta_pri(1), true); // N(mean, sd)
  ...
  vector<Type> fe_iUrbanMICS = X_betaUrbanMICS * beta + diffUrbMICS;
  vector<Type> fe_iRuralMICS = X_betaRuralMICS * beta + diffRurMICS;
