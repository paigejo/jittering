// BYM2 constrained (2n-2) + GH nuggets — OPTIMIZED v2
// Same model as modM_BYM2_GH.cpp but with:
//   - base_eta computed once (not per GH node)
//   - inline binomial (no dbinom_robust overhead)
//   - lchoose factored out as DATA (not on AD tape)
//   - no if-branching on AD tape
//   - logSumExp done row-wise on nObs x Q matrix
...
Type objective_function<Type>::operator() ()
{
  ...
  // ── Urban: build nUrb x Q matrix of log(GH-weighted mixture lik) ──
  ...
  // logSumExp per obs over Q nodes
  for(int i = 0; i < nUrb; i++) {
