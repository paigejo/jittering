...
Type objective_function<Type>::operator() ()
{
  ...
  for (int obsI = 0; obsI < num_iUrbanMICS; obsI++) {
    ...
    for (int intI = 0; intI < n_integrationPointsUrbanMICS; intI++) {
      ...
      latentFieldUrbMICS(thisIndex) = fe_iUrbanMICS(thisIndex) + w_bym2Star(areaidxlocUrbanMICS(thisIndex)) + nuggetUrbMICS(obsI);
  ...
  for (int obsI = 0; obsI < num_iRuralMICS; obsI++) {
    ...
    for (int intI = 0; intI < n_integrationPointsRuralMICS; intI++) {
      ...
      // latent field estimate at each obs
      latentFieldRurMICS(thisIndex) = fe_iRuralMICS(thisIndex) + w_bym2Star(areaidxlocRuralMICS(thisIndex)) + nuggetRurMICS(obsI);
  ...
  for (int obsI = 0; obsI < num_iUrbanDHS; obsI++) {
    ...
    for (int intI = 0; intI < n_integrationPointsUrbanDHS; intI++) {
      ...
      // latent field estimate at each obs
      latentFieldUrbDHS(thisIndex) = fe_iUrbanDHS(thisIndex) + w_bym2Star(areaidxlocUrbanDHS(thisIndex)) + nuggetUrbDHS(obsI);
  ...
  for (int obsI = 0; obsI < num_iRuralDHS; obsI++) {
    ...
    for (int intI = 0; intI < n_integrationPointsRuralDHS; intI++) {
      ...
      // latent field estimate at each obs
      latentFieldRurDHS(thisIndex) = fe_iRuralDHS(thisIndex) + w_bym2Star(areaidxlocRuralDHS(thisIndex)) + nuggetRurDHS(obsI);
