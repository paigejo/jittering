source("code/setup.R")
# DHS+MICS fusion FE+nugget GH model: fit, predictions and plots
...
KDHSu <- 16
KDHSr <- 21
fitCacheFile = "savedOutput/global/fitFEMD_final.RData"
if(file.exists(fitCacheFile)) {
    load(fitCacheFile)
} else {
    res = fitFEMD(datDHS=ed, datMICS=edMICS, KMICS=KMICS, KDHSurb=KDHSu, KDHSrur=KDHSr, Qgh=Qgh, getSDs=TRUE, verbose=FALSE)
...
plotPreds(SD0, obj, popMat=popMatNGAThresh,
          ...
          plotNameRoot="edFE_DM_final", plotNameRootAreal="Strat")
plotPreds(SD0, obj, popMat=popMatNGAThresh,
          gridPreds=gridPreds, arealPreds=admin1Preds,
          plotNameRoot="edFE_DM_final", plotNameRootAreal="Admin1")
plotPreds(SD0, obj, popMat=popMatNGAThresh,
          gridPreds=gridPreds, arealPreds=admin2Preds,
          plotNameRoot="edFE_DM_final", plotNameRootAreal="Admin2")
plotPreds(SD0, obj, popMat=popMatNGAThresh,
          gridPreds=gridPreds, arealPreds=NULL,
          plotNameRoot="edFE_DM_final")
